...
```bash
INIT=$(node "E:/Proyectos Github/brahcaciones/.gemini/get-shit-done/bin/gsd-tools.cjs" init plan-phase "$PHASE")
...
```bash
PHASE_INFO=$(node "E:/Proyectos Github/brahcaciones/.gemini/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}")
```
...
```bash
node "E:/Proyectos Github/brahcaciones/.gemini/get-shit-done/bin/gsd-tools.cjs" state planned-phase --phase "${PHASE}" --name "${phase_name}" --plans "${PLAN_COUNT}"
```
