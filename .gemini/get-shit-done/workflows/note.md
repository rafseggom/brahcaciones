...
- **Global scope**: `E:/Proyectos Github/brahcaciones/.gemini/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `E:/Proyectos Github/brahcaciones/.gemini/notes/`)
...
2. Glob `E:/Proyectos Github/brahcaciones/.gemini/notes/*.md` (if directory exists) — global notes
...
Global (E:/Proyectos Github/brahcaciones/.gemini/notes/):
...
2. **No `.planning/`**: Falls back to global `E:/Proyectos Github/brahcaciones/.gemini/notes/` — works in any directory
